...
export default function WeeklyKmChart({ activities, fromDate }: Props) {
  const weekMap = new Map<string, number>()
  for (const a of activities) {
    if (new Date(a.startDate) < fromDate) continue
    const week = getWeekStart(a.startDate)
    weekMap.set(week, (weekMap.get(week) ?? 0) + a.distanceMeters / 1000)
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={160}>
      <BarChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -18 }}>
        ...
        <Bar dataKey="km" fill="#34D399" radius={[3, 3, 0, 0]} maxBarSize={32} />
      </BarChart>
    </ResponsiveContainer>
  )
}
